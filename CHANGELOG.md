...
### 🖨️ 2. แก้ไขปัญหาตั้งค่าเครื่องพิมพ์ไม่ยอมจำค่า (Printer Settings Persistence Fix)
* **ปัญหาที่พบ:** เมื่อผู้ใช้เปลี่ยนการตั้งค่าเครื่องพิมพ์เครื่องจริง (เช่น เปลี่ยน `printer_type` เป็น `thermal` เพื่อสั่งเด้งลิ้นชัก/ตัดกระดาษ หรือเปลี่ยนขนาดกระดาษ) ระบบมักจะย้อนการบันทึกกลับไปเป็นค่าเริ่มต้นของ GDI (`windows` + `58mm`) ทุกครั้งที่ทำการติดต่อฐานข้อมูล ทำให้ไม่จำค่าที่ผู้ใช้เลือกจริง
* **ไฟล์ที่แก้ไข:** [database/db_manager.py](file:///c:/Users/admin/Documents/store-pos/database/db_manager.py)
* **จุดที่ทำการแก้ไข:**
  * ปรับฟังก์ชันการตั้งค่าเริ่มต้น `_check_and_auto_init` ให้หลีกเลี่ยงการเขียนทับข้อมูลหากมีค่า `printer_type` และ `paper_size` บันทึกอยู่ในระบบอยู่แล้ว
  * ระบบจะบังคับตั้งค่าเริ่มต้น (GDI + 58mm) สำหรับเครื่องรุ่น XP-58 **เฉพาะในกรณีที่ติดตั้งฐานข้อมูลใหม่หรือค่าการตั้งค่าเดิมเป็นค่าว่างเท่านั้น** ป้องกันการแอบเขียนทับการตั้งค่าที่ผู้ใช้กำหนดเองอย่างถาวร
