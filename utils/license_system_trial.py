# -*- coding: utf-8 -*-
"""
Hardware ID & License System - 15-Day Trial Version
ระบบตรวจสอบการทดลองใช้งาน 15 วัน โดยใช้วิธีเก็บรักษาวันที่เริ่มใช้อย่างปลอดภัย
"""
...
class HardwareID:
    """จัดการ Hardware ID (เหมือนเวอร์ชันปกติเพื่อให้ระบบอื่นๆ เรียกใช้ได้)"""
...
class LicenseManager:
    """จัดการสิทธิ์การใช้งานสำหรับเวอร์ชันทดลอง 15 วัน"""
    
    TRIAL_FILE = Path("data/.trial")
    LICENSE_FILE = Path("data/.license") # คงไว้เพื่อความเข้ากันได้
    MANUAL_LICENSE_PATH = None
    
    @staticmethod
    def set_manual_license_path(path):
        pass # ไม่ต้องใช้งานในตัวทดลอง
    ...
    @staticmethod
    def generate_license_key(hwid, expire_days=15, features=None):
        return "TRIAL-VERSION-15-DAYS-FREE"
        ...
        try:
            ...
            if days_left <= 0:
                return {
                    ...
                    'message': (
                        f"ระยะเวลาทดลองใช้งานฟรี 15 วันหมดลงแล้ว!\n\n"
    ...
    def save_license(license_key):
        messagebox.showinfo("ทดลองใช้งาน", "โปรแกรมนี้เป็นเวอร์ชันทดลองใช้ฟรี 15 วัน ไม่จำเป็นต้องระบุคีย์สิทธิ์การใช้งาน")
        return True
    ...
    @staticmethod
    def delete_license():
        messagebox.showwarning("ทดลองใช้งาน", "ไม่สามารถถอดถอนสิทธิ์ในเวอร์ชันทดลองใช้ฟรี 15 วันได้")
        return False
            
    @staticmethod
    def transfer_license():
        messagebox.showwarning("ทดลองใช้งาน", "ไม่สามารถโอนย้ายสิทธิ์ในเวอร์ชันทดลองใช้ฟรี 15 วันได้")
    ...
    def verify_system_clock():
        ...
        try:
            ...
            log_error(f"Error checking system clock in trial: {e}")
            return True, str(e)
    ...
    def _write_trial_file(dt):
        ...
        try:
            ...
            log_error(f"Error writing trial file: {e}")
            return False
    ...
    def _get_trial_db_date():
        ...
        try:
            db = DatabaseManager()
            db.connect()
            result = db.fetch_one("SELECT setting_value FROM settings WHERE setting_key = 'trial_start_date'")
            db.disconnect()
            if result:
                # ถอดรหัส Base64 ของวันที่ในฐานข้อมูลเพื่อความปลอดภัย
                decoded = base64.b64decode(result['setting_value'].encode()).decode()
                return datetime.strptime(decoded, "%Y-%m-%d %H:%M:%S")
            return None
        except Exception as e:
            log_error(f"Error getting trial DB date: {e}")
            return None
    ...
    def _write_trial_db_date(dt):
        ...
        try:
            ...
            db.execute("""
                ...
                VALUES ('trial_start_date', ?)
            """, (encoded,))
            db.disconnect()
            return True
        except Exception as e:
            log_error(f"Error writing trial DB date: {e}")
            return False

    @staticmethod
    def check_activation():
        """ตรวจสอบเวลาทดลองใช้ 15 วัน"""
        # 1. ตรวจสอบเวลาเครื่องถูกโกงหรือไม่
        ...
        # 2. อ่านข้อมูลวันเริ่มต้นจาก File และ DB เพื่อความปลอดภัยแบบสองชั้น
        ...
        if file_date and db_date:
            # ถ้ามีทั้งสองค่า ให้ตรวจสอบความสอดคล้อง
            # หากมีการแก้ไขตัวใดตัวหนึ่งหรือโกง ให้ดึงค่าที่เก่าที่สุด (ป้องกันการยืดเวลาออกไป)
            if file_date < db_date:
                trial_start = file_date
                # ซิงค์ค่า db ให้ตรงกับค่าที่เก่ากว่า
                LicenseManager._write_trial_db_date(file_date)
            else:
                trial_start = db_date
                # ซิงค์ค่า file ให้ตรงกับค่าที่เก่ากว่า
                LicenseManager._write_trial_file(db_date)
        elif file_date:
            # ถ้ามีแค่ไฟล์ ให้เขียนลง DB
            trial_start = file_date
            LicenseManager._write_trial_db_date(file_date)
        elif db_date:
            # ถ้ามีแค่ใน DB ให้เขียนลงไฟล์
            trial_start = db_date
            LicenseManager._write_trial_file(db_date)
        else:
            # เริ่มใช้งานครั้งแรก! บันทึกเวลาปัจจุบันลงทั้งสองจุด
            trial_start = current_time
            LicenseManager._write_trial_file(current_time)
            LicenseManager._write_trial_db_date(current_time)
            
        # 3. ตรวจสอบการย้อนเวลาเพื่อยืดอายุ (เทียบเวลากับวันเริ่มต้นทดลอง)
        ...
        expire_date = (trial_start + timedelta(days=15)).date()
